...
async def send_notification_about_precipitation(bot, sender):
...
async def send_reminder(bot, sender, full_name):
...
async def demo(text, sender, message, full_name, bot):
