...
async def weather_forecast(response: Dict[str, Any], user: str) -> str:
    ...
    for data in response['list']:
        ...
        date_ = dt.strptime(data['dt_txt'], '%Y-%m-%d %H:%M:%S').date()
        time = dt.strptime(data['dt_txt'], '%Y-%m-%d %H:%M:%S').time().hour
    ...
    for date, data in daily_data.items():
        ...
        if max_rain_prob > 0:
            header += f"{weekday_names[date.weekday()]}: <b>{min_temp:.0f}°-{max_temp:.0f}°</b> ({max_rain_prob * 100:.0f}%) {weather_icons[max(data['weather_icons'], key=data['weather_icons'].count)]}\n"
        else:
...
async def check_precipitation(city_name: str) -> Optional[str]:
    ...
    try:
        ...
        reminder_str_telegram, rain_probabilities = await weather_data_extractor(response.json())
        if len(rain_probabilities) > 0:
            return reminder_str_telegram
    ...
    for i in data['list']:
        if i['pop'] * 100 > 50: # Проверка вероятность дождя больше 50% ?
            ...
            if rain_forecast:
                ...
                rain_forecast = False
    return reminder_str_telegram, rain_probabilities